...
class ResumeAnalysis(SQLModel, table=(True)):
    ...
    tb_matching_skills: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_missing_skills: List[str] = Field(default=[], sa_column=Column(JSON))
    experience_level: str = Field(max_length=20)
    tb_matching_experience: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_experience_gaps: List[str] = Field(default=[], sa_column=Column(JSON))
    education_level: str = Field(max_length=20)
    tb_education_highlights: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_matching_education: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_missing_education: List[str] = Field(default=[], sa_column=Column(JSON))
    ...
    tb_strengths: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_weaknesses: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_red_flags: List[str] = Field(default=[], sa_column=Column(JSON))
    tb_cultural_fit_indicators: List[str] = Field(default=[], sa_column=Column(JSON))
    salary_expectation_alignment: str = Field(max_length=20)
    onboarding_priority: str = Field(max_length=20)
    tb_interview_focus_areas: List[str] = Field(default=[], sa_column=Column(JSON))
